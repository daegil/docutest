...
이 문서는 `docs/` 디렉토리의 **프로젝트 문서** 작성 규칙을 정의합니다.\
계획 → 실행 → 결과 → 회고 흐름을 명확히 기록하고,\
시간 순서와 의사결정 맥락을 쉽게 추적할 수 있도록 설계되어 있습니다.

> **적용 범위**: 이 규칙은 `docs/` 디렉토리에만 적용됩니다.\
> `wiki/` 디렉토리는 `CLAUDE.md`의 Wiki 규칙을 따릅니다.

LLM은 이 저장소의 문서를 생성/수정할 때 아래 규칙을 반드시 준수해야 합니다.
...
-   [ ] `slug`가 URL-friendly한지 확인했는가 (kebab-case, 영문 소문자)
-   [ ] tags가 최소 1개 이상 있는가
-   [ ] draft 상태가 맞는가
-   [ ] 필수 섹션이 모두 포함되었는가
